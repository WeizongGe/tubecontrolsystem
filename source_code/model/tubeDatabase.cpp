...
tubeDatabase::tubeDatabase(QObject *parent) : QObject(parent)
{
    ...
    dbOutPath = QCoreApplication::applicationDirPath() + "/data_output";
    QDir dir;
    if (!dir.exists(dbDirPath)) {
        dir.mkpath(dbDirPath);
    }
    if (!dir.exists(dbOutPath)) {
        dir.mkpath(dbOutPath);
    }
...
void tubeDatabase::init()
{
    ...
    m_SensorDataQuerry.prepare("INSERT INTO force_sensor_data (time, fA, fB, fC, fD, fE, fF, fABC, fDEF) "
                                                        "VALUES (:time, :fA, :fB, :fC, :fD, :fE, :fF, :fABC, :fDEF)");

    m_motionParameterQuerry = QSqlQuery(m_tubeDatabase);
    m_motionParameterQuerry.prepare("INSERT INTO tube_motion_parameter "
                                    "(time, velocityA, velocityB, velocityC, velocityD, velocityE, velocityF, torA, torB, torC, torABC, torD, torE, torF, torDEF, curA, curB, curC, curD, curE, curF)"
                                    "VALUES "
                                    "(:time, :velocityA, :velocityB, :velocityC, :velocityD, :velocityE, :velocityF, :torA, :torB, :torC, :torABC, :torD, :torE, :torF, :torDEF, :curA, :curB, :curC, :curD, :curE, :curF)");
...
void tubeDatabase::saveSensorData(Force ft)
{
    ...
    m_SensorDataQuerry.bindValue(":fF", -ft.FF);
    m_SensorDataQuerry.bindValue(":fF", -ft.FF);
    m_SensorDataQuerry.bindValue(":fABC", -ft.FA-ft.FB-ft.FC);
    m_SensorDataQuerry.bindValue(":fDEF", -ft.FD-ft.FE-ft.FF);
...
void tubeDatabase::savetubeMotionParameter(const tubeMotionParameter &mp)
{
    ...
    m_motionParameterQuerry.bindValue(":torABC", -mp.torqueA-mp.torqueB-mp.torqueC);
    ...
    m_motionParameterQuerry.bindValue(":torDEF", -mp.torqueD-mp.torqueE-mp.torqueF);
