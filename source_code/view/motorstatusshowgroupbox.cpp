    ...
    m_app(tubeControlSystemApp::instance())
{
    ...
    connect(m_app->sensorDataCollector, &SensorDataCollector::SensorDataUpdated,
            this, &motorstatusshowGroupBox::updateSensorData2, Qt::DirectConnection);// 连接力传感器更新的信号
...
void motorstatusshowGroupBox::updatetubeMotionParameter(tubeMotionParameter mp)
{
    ...
    {
//        qDebug()<<"motion_count0:"<<m_AVelocitySeries->count();
...
//        qDebug()<<"motion_count1:"<<m_AVelocitySeries->count();
        // 更新曲线图
...
//        qDebug()<<"motion_count2:"<<m_AVelocitySeries->count();
...
void motorstatusshowGroupBox::updateSensorData(Force ft)
{
    text_display_num2++;
    // 更新曲线图
    if(text_display_num2 == 2)
    {
        if(m_FASeries->count() >= 200)
        {
             ...
             m_FABCSeries->removePoints(0, 1);
             m_FDEFSeries->removePoints(0, 1);
        }
        m_FASeries->append(QPointF(ft.time, -ft.FA));
        m_FBSeries->append(QPointF(ft.time, -ft.FB));
        m_FCSeries->append(QPointF(ft.time, -ft.FC));
        m_FDSeries->append(QPointF(ft.time, -ft.FD));
        m_FESeries->append(QPointF(ft.time, -ft.FE));
        m_FFSeries->append(QPointF(ft.time, -ft.FF));
        m_FABCSeries->append(QPointF(ft.time, -ft.FA-ft.FB-ft.FC));
        m_FDEFSeries->append(QPointF(ft.time, -ft.FD-ft.FE-ft.FF));
        //貌似是并行计算，append没跑完就往下进行，容易触发index > m_series->count()的错误
        Sleep(20);
        // 实现横轴随时间滚动
        // 当前时间映射到绘图区域的坐标
        ...
        ui->TotalAcheckBox->setText(QString::number(-ft.FA-ft.FB-ft.FC, 'f', 4));
        ui->TotalDcheckBox->setText(QString::number(-ft.FD-ft.FE-ft.FF, 'f', 4));
//        QWidget::update();
        text_display_num2 = 0;
     }
//    Sleep(20);
...
void motorstatusshowGroupBox::updateSensorData2(Force ft)
{
    mutex.lock();
    text_display_num2++;
    if(text_display_num2 == 2){

//        qDebug()<<"sensor_size0:"<<m_FAbuffer.size();
        if(m_FAbuffer.size()>=60)
        {
             ...
             m_FABCbuffer.pop_front();
             m_FDEFbuffer.pop_front();
        }
//        qDebug()<<"sensor_size1:"<<m_FAbuffer.size();
        ...
        m_FABCbuffer.append(QPointF(ft.time, -ft.FA-ft.FB-ft.FC));
        m_FDEFbuffer.append(QPointF(ft.time, -ft.FD-ft.FE-ft.FF));
//        qDebug()<<"sensor_size2:"<<m_FAbuffer.size();
        // 更新曲线图
        m_FASeries->replace(m_FAbuffer);
        m_FBSeries->replace(m_FBbuffer);
        m_FCSeries->replace(m_FCbuffer);
        m_FDSeries->replace(m_FDbuffer);
        m_FESeries->replace(m_FEbuffer);
        m_FFSeries->replace(m_FFbuffer);
        m_FABCSeries->replace(m_FABCbuffer);
        m_FDEFSeries->replace(m_FDEFbuffer);

//        qDebug()<<"sensor_count:"<<m_FASeries->count();
...
void motorstatusshowGroupBox::initTubeChart()
{
    ...
    m_FABCSeries = new QLineSeries(this);
    m_FABCSeries->setName("FABC");
    m_FABCSeries->setVisible(ui->TotalAcheckBox->isChecked());
    m_chart->addSeries(m_FABCSeries);
    m_FABCSeries->attachAxis(m_axisX);
    m_FABCSeries->attachAxis(m_axisFY);

    m_FDEFSeries = new QLineSeries(this);
    m_FDEFSeries->setName("FDEF");
    m_FDEFSeries->setVisible(ui->TotalDcheckBox->isChecked());
    m_chart->addSeries(m_FDEFSeries);
    m_FDEFSeries->attachAxis(m_axisX);
    m_FDEFSeries->attachAxis(m_axisFY);
}
...
void motorstatusshowGroupBox::on_TotalAcheckBox_stateChanged(int arg1)
{
    m_FABCSeries->setVisible(arg1);
}

void motorstatusshowGroupBox::on_TotalDcheckBox_stateChanged(int arg1)
{
    m_FDEFSeries->setVisible(arg1);
}
...
void motorstatusshowGroupBox::on_pushButton_clicked()
{
    ...
    bool Lock = ui->SetABtext->isReadOnly();
    QString color =Lock? "(255, 255, 255)":"(200, 200, 200)";
    ui->SetABtext->setReadOnly(!Lock);
    ui->SetABtext->setStyleSheet("background-color: rgb" + color);
    ui->SetBCtext->setReadOnly(!Lock);
    ui->SetBCtext->setStyleSheet("background-color: rgb" + color);
    ui->SetCAtext->setReadOnly(!Lock);
    ui->SetCAtext->setStyleSheet("background-color: rgb" + color);
    ui->SetDEtext->setReadOnly(!Lock);
    ui->SetDEtext->setStyleSheet("background-color: rgb" + color);
    ui->SetEFtext->setReadOnly(!Lock);
    ui->SetEFtext->setStyleSheet("background-color: rgb" + color);
    ui->SetFDtext->setReadOnly(!Lock);
    ui->SetFDtext->setStyleSheet("background-color: rgb" + color);
