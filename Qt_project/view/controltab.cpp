    ...
    mApp(tubeControlSystemApp::instance())
{
    ...
    connect(this, &ControlTab::SetFlag1,
            mApp->tubemotionController, &tubeMotionController::GetFlag1, Qt::AutoConnection);
    connect(this, &ControlTab::SetFlag2,
            mApp->tubemotionController, &tubeMotionController::GetFlag2, Qt::AutoConnection);
}
...
//        PassTor(0, 0, 0, 0, 0, 0);
        mApp->tubemotionController->CSTmodeSetVel(0);
        ui->actualVelLimit->setText(QString::number(0, 'g', 2));
...
void ControlTab::on_Dextension_pressed()
{
    if(ui->singleSetModeTab->currentIndex())
    {
        PassTor(0, 0, 0, ui->torCInput->text().toDouble(), 0, 0);
    }
    else
    {
        PassVel(0, 0, 0, ui->velCInput->text().toDouble(), 0, 0);
    }
}
...
void ControlTab::on_Eextension_pressed()
{
    if(ui->singleSetModeTab->currentIndex())
    {
        PassTor(0, 0, 0, 0, ui->torCInput->text().toDouble(), 0);
    }
    else
    {
        PassVel(0, 0, 0, 0, ui->velCInput->text().toDouble(), 0);
    }
}
...
void ControlTab::on_Fextension_pressed()
{
    if(ui->singleSetModeTab->currentIndex())
    {
        PassTor(0, 0, 0, 0, 0, ui->torCInput->text().toDouble());
    }
    else
    {
        PassVel(0, 0, 0, 0, 0, ui->velCInput->text().toDouble());
    }
}
...
//    ui->groupTorLock->setEnabled(!flag);
//    ui->torSlider->setEnabled(!flag);
//    ui->torVelSlider->setEnabled(!flag);
}
...
{
    //qDebug()<<arg1;
    arg1? emit SetFlag1(true): emit SetFlag1(false) ;
...
{
    arg1? emit SetFlag2(true): emit SetFlag2(false) ;
